        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <div class="card strpied-tabled-with-hover">
                    ...
                    <div class="card-body">
                        <form action="{{ route('dataawal.update', $alternatif->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('put')
                            <div class="mb-3">
                                ...
                                {{--                                    @endforeach--}}
                                {{--                                </select>--}}
                                <input name="nama" type="text" value="{{$alternatif->nama_alternative}}" class="form-control">
                            </div>
                                            ...
                                            @foreach ($item->sub_kriteria as $index => $sub_kriteria)
                                                <option value="{{$sub_kriteria->id}}"
                                                        ...
                                                        @if( in_array($sub_kriteria->id,$sub_kriteria_s) )
                                                            selected
                                                        @endif
                                                >{{$sub_kriteria->nama}} - {{ $sub_kriteria->bobot }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
