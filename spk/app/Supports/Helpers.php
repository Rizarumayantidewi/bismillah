    ...
    function proses_pengalian_bobot($bobot,$nilai){
      ...
      foreach ($nilai as $key => $value) {
        ...
        $hasill[] = [
          ...
          'kriteria'    => $nilai[$key]['kriteria_id'],
...
if ( ! function_exists('proses_normalisasi') )
{
  function proses_normalisasi($ciMaksMin,$hasil){
    ...
    foreach ($hasil as $index => $item) {
      ...
      $kriteriaHasil = $hasil[$index]['kriteria_id'] ;

      foreach ($ciMaksMin as $key => $value) {
        $nilaiMaksMin = $value['nilai'] ;
        $attribute = $value['attribute'];
        $kriteriaMaks = $value['kriteria'];

        if ($kriteriaHasil == $kriteriaMaks) {
